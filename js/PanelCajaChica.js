			...
			function ActivarMenu($id){
			 ...
			 function AbrirCajaChica(){
				 ...
				 $.post('controlador/Ccaja.php',{accion:'VAL_USER',user:$user},function(datitos){
					   if(datitos==0){$.post('Adm_AbrirCaja.php',{user:$user},function(datitos){$("#IdCuerpoCajaChica").html(datitos); 
					    })}
					   if(datitos==1){
				...
				$cod_ingreso=$("#HiCodIngreso").val();
				 $.post('controlador/Ccaja.php',{accion:'VAL_USER',user:$user},function(datitos){
					   ...
					   if(datitos==0 ){swal("Usuario No tiene Caja Inicializada", "Error", "warning"); ActivarMenu(0);
					        $("#IdCuerpoCajaChica").html("");}
					   if(datitos==1){
						  $.post('Caja_RecibosPro.php',{},function(datitos){$("#IdCuerpoCajaChica").html(datitos); })
						}  
				  })
			 }
				...
				$cod_ingreso=$("#HiCodIngreso").val();
				 $.post('controlador/Ccaja.php',{accion:'VAL_USER',user:$user},function(datitos){
					   ...
					   if(datitos==0 ){swal("Usuario No tiene Caja Inicializada", "Error", "warning"); ActivarMenu(0);
					        $("#IdCuerpoCajaChica").html("");}
					   if(datitos==1){
						  $.post('Caja_Resumen.php',{},function(datitos){$("#IdCuerpoCajaChica").html(datitos); })
						}  
				  })
			 }
