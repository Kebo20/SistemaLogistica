...
class Contabilidad
{
    ...
    {
        $ocado = new cado();
        $sql = "select * from cont_centro_costos where estado=0  and (codigo like '%$q%' or descripcion like '%$q%' ) order by codigo asc  limit $inicio,$fin";
        $ejecutar = $ocado->ejecutar($sql);
        return $ejecutar;
    }
    ...
    function ListarEmpresa($q, $inicio, $fin)
    {
        ...
        $sql = "select * from cont_empresa where  estado=0  and (nombre like '%$q%' or ruc like '%$q%' ) order by nombre asc  limit $inicio,$fin";
        $ejecutar = $ocado->ejecutar($sql);
        return $ejecutar;
    }
    ...
    {
        $ocado = new cado();
        $sql = "select p.id,p.codigo,p.nombre,p.haber,p.debe,p.tipo,p.id_tipo_auxiliar,p.tasa_cambio,p.estado,p.movimiento,p.estado_financiero,a.nombre as
           tipo_auxiliar from cont_plan_cuentas p INNER JOIN cont_tipo_auxiliares a on a.id=p.id_tipo_auxiliar
            where p.estado=0  and (p.codigo like '%$q%' or p.nombre like '%$q%'  ) order by p.codigo asc limit $inicio,$fin";
        $ejecutar = $ocado->ejecutar($sql);
        return $ejecutar;
    }
    ...
    function ListarSucursal($q, $inicio, $fin, $id_empresa)
    {
        ...
        $sql = "select s.*,e.nombre as nombre_empresa from cont_sucursal s inner join cont_empresa e on e.id=s.empresa 
         where s.empresa=$id_empresa and s.estado=0  and (s.nombre like '%$q%' ) order by nombre asc  limit $inicio,$fin ";
        $ejecutar = $ocado->ejecutar($sql);
        return $ejecutar;
    }
    ...
    function ListarTipoAuxiliares($q, $inicio, $fin)
    {
        $ocado = new cado();
        $sql = "select * from cont_tipo_auxiliares where estado=0 and nombre like '%$q%'  limit $inicio,$fin ";
        $ejecutar = $ocado->ejecutar($sql);
        return $ejecutar;
    }
    ...
    function ListarTipoCambio($q, $inicio, $fin)
    {
        $ocado = new cado();
        $sql = "select * from cont_tipo_cambio where estado=0 and fecha like '%$q%' order by fecha desc limit $inicio,$fin";
        $ejecutar = $ocado->ejecutar($sql);
        return $ejecutar;
    }
    ...
    function ListarTipoDocumentos($q, $inicio, $fin)
    {
        $ocado = new cado();
        $sql = "select * from cont_tipo_documentos where estado=0 and (codigo like '%$q%' or nombre like '%$q%'  ) order by nombre asc limit $inicio,$fin ";
        $ejecutar = $ocado->ejecutar($sql);
        return $ejecutar;
    }
