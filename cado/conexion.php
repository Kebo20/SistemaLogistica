<?php 
 class cado{
	  function conectar(){
	   ...
	   $db = new PDO('mysql:host=localhost;dbname=contable;charset=utf8','root','');
	  ...
	  function conectar2(){
	   try {
		date_default_timezone_set('America/Lima');
	  $db = new PDO('mysql:host=localhost;dbname=bd_lab','root','',array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \'UTF8\'',PDO::MYSQL_ATTR_INIT_COMMAND => "SET lc_time_names=es_PE"));
	  //  
		$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		 return $db;
		 }catch (PDOException $e) {
